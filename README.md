...
[![Version](https://img.shields.io/badge/version-5.2.52-blue.svg)](https://github.com/Dicklesworthstone/ultimate_bug_scanner)
...
- **Go** – Category 5/17 now run a Go AST walker (`modules/helpers/resource_lifecycle_go.go`) that detects `context.With*` calls missing cancel, `time.NewTicker/NewTimer` without `Stop`, `os.Open/sql.Open` without `Close`, and mutex `Lock`/`Unlock` symmetry. Category 9 also tracks request query/header/form/framework values into response headers unless they strip or reject CR/LF, and tracks redirect targets into `http.Redirect`, framework `Redirect` calls, and `Location` headers unless they pass through same-origin or explicit allow-list validation. Findings come straight from the AST/resource helper or taint pass positions, so “ticker missing Stop()” and header/open-redirect lines map to exact `file:line` references instead of coarse regex summaries.
