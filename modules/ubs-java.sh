#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════════════
# JAVA ULTIMATE BUG SCANNER v1.2.2 - Industrial-Grade Java 21+ Code Analysis
# ═══════════════════════════════════════════════════════════════════════════
...
run_path_traversal_checks() {
  ...
  while IFS=$'\t' read -r tag a b c; do
    case "$tag" in
      __COUNT__)
        if [[ "$a" -gt 0 ]]; then
          print_finding "critical" "$a" "Request-derived path reaches file read/write/serve sink" "Validate paths with canonical/normalized startsWith checks or reduce upload names to a basename before opening, serving, or deleting files"
        else
...
run_open_redirect_checks() {
  ...
  while IFS=$'\t' read -r tag a b c; do
    case "$tag" in
      __COUNT__)
        if [[ "$a" -gt 0 ]]; then
          print_finding "critical" "$a" "Unvalidated redirect from request data" "Validate redirect targets with same-origin relative paths or an explicit host allow-list before sendRedirect, redirect views, or Location headers"
        else
...
run_response_header_injection_checks() {
  ...
  while IFS=$'\t' read -r tag a b c; do
    case "$tag" in
      __COUNT__)
        if [[ "$a" -gt 0 ]]; then
          print_finding "critical" "$a" "Request-controlled value reaches HTTP response header" "Reject or strip CR/LF, URL-encode filename fragments, or route values through a header-safe helper before setHeader/addHeader/header/headers.append"
        else
...
run_outbound_url_checks() {
  ...
  while IFS=$'\t' read -r tag a b c; do
    case "$tag" in
      __COUNT__)
        if [[ "$a" -gt 0 ]]; then
          print_finding "critical" "$a" "Request-derived URL reaches outbound HTTP client" "Validate outbound URLs with explicit scheme and host allow-lists before building or sending client requests"
        else
