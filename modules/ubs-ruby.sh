#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════════════
# RUBY ULTIMATE BUG SCANNER v2.0.1 (Bash) - Industrial-Grade Code Analysis
# ═══════════════════════════════════════════════════════════════════════════
...
run_outbound_url_checks() {
  ...
  while IFS=$'\t' read -r tag a b c; do
    case "$tag" in
      __COUNT__)
        if [[ "$a" -gt 0 ]]; then
          print_finding "critical" "$a" "Request-derived URL reaches outbound HTTP client" "Validate outbound URLs with explicit scheme and host allow-lists before using Net::HTTP, URI.open, Faraday, HTTParty, or RestClient"
        else
...
run_open_redirect_checks() {
  ...
  while IFS=$'\t' read -r tag a b c; do
    case "$tag" in
      __COUNT__)
        if [[ "$a" -gt 0 ]]; then
          print_finding "critical" "$a" "Unvalidated redirect from request data" "Validate redirect targets with local-url checks or explicit scheme and host allow-lists before redirect_to, redirect, or Location headers"
        else
