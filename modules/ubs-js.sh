...
print_subheader "async React effect callbacks"
async_effect_report=$(python3 - "$PROJECT_DIR" <<'PY' 2>/dev/null
import os
import re
import sys
from pathlib import Path
...
async_effect_count=$(printf '%s\n' "$async_effect_report" | head -n1 | awk 'END{print $0+0}')
async_effect_samples=$(printf '%s\n' "$async_effect_report" | tail -n +2)
if [ "$async_effect_count" -gt 0 ]; then
  print_finding "warning" "$async_effect_count" "async React effect callback" "Do not pass an async function directly to useEffect; define and call an inner async function instead"
  sample_limit=3
  while IFS=$'\t' read -r sample_path sample_line sample_text; do
    [ -z "$sample_path" ] && continue
    print_code_sample "$sample_path" "$sample_line" "$sample_text"
...
async_map_ignored_count=$(printf '%s\n' "$async_map_ignored_report" | head -n1 | awk 'END{print $0+0}')
async_map_ignored_samples=$(printf '%s\n' "$async_map_ignored_report" | tail -n +2)
if [ "$async_map_ignored_count" -gt 0 ]; then
  print_finding "warning" "$async_map_ignored_count" "async map callback result is ignored" "Await Promise.all(array.map(async ...)) or use for...of so the generated promises are observed"
  sample_limit=3
  while IFS=$'\t' read -r sample_path sample_line sample_text; do
    [ -z "$sample_path" ] && continue
    print_code_sample "$sample_path" "$sample_line" "$sample_text"
...
print_subheader "async callbacks passed to reduce"
async_reduce_report=$(python3 - "$PROJECT_DIR" <<'PY' 2>/dev/null
import os
import re
import sys
from pathlib import Path
...
for path in candidates:
    ...
    for idx, line in enumerate(lines):
        ...
        if 'ubs:ignore' in callback_text:
            continue
        match = async_reduce_re.search(callback_text)
        if not match:
            continue
        try:
            rel = path.relative_to(sample_root)
            ...
            rel = path
        issues.append((str(rel), idx + 1, match.group(1), stripped.replace('\t', ' ')))
...
async_reduce_count=$(printf '%s\n' "$async_reduce_report" | head -n1 | awk 'END{print $0+0}')
async_reduce_samples=$(printf '%s\n' "$async_reduce_report" | tail -n +2)
if [ "$async_reduce_count" -gt 0 ]; then
  print_finding "warning" "$async_reduce_count" "async reduce callback returns a Promise accumulator" "Resolve async values before reduce, or await the accumulator explicitly and return a Promise intentionally"
  sample_limit=3
  while IFS=$'\t' read -r sample_path sample_line sample_method sample_text; do
    [ -z "$sample_path" ] && continue
    print_code_sample "$sample_path" "$sample_line" "$sample_text"
    sample_limit=$((sample_limit - 1))
    [ "$sample_limit" -le 0 ] && break
...
print_subheader "async callbacks passed to array predicates"
async_predicate_report=$(python3 - "$PROJECT_DIR" <<'PY' 2>/dev/null
import os
import re
import sys
from pathlib import Path
...
async_predicate_count=$(printf '%s\n' "$async_predicate_report" | head -n1 | awk 'END{print $0+0}')
async_predicate_samples=$(printf '%s\n' "$async_predicate_report" | tail -n +2)
if [ "$async_predicate_count" -gt 0 ]; then
  print_finding "warning" "$async_predicate_count" "async array predicate callback is not awaited" "Resolve async checks before filter/some/every/find, then use a synchronous predicate over resolved values"
  sample_limit=3
  while IFS=$'\t' read -r sample_path sample_line sample_method sample_text; do
    [ -z "$sample_path" ] && continue
    print_code_sample "$sample_path" "$sample_line" "$sample_text"
    sample_limit=$((sample_limit - 1))
    [ "$sample_limit" -le 0 ] && break
  done <<<"$async_predicate_samples"
fi

print_subheader "async comparators passed to sort"
async_sort_comparator_report=$(python3 - "$PROJECT_DIR" <<'PY' 2>/dev/null
import os
import re
import sys
from pathlib import Path
...
async_sort_comparator_count=$(printf '%s\n' "$async_sort_comparator_report" | head -n1 | awk 'END{print $0+0}')
async_sort_comparator_samples=$(printf '%s\n' "$async_sort_comparator_report" | tail -n +2)
if [ "$async_sort_comparator_count" -gt 0 ]; then
  print_finding "warning" "$async_sort_comparator_count" "async sort comparator returns a Promise" "Precompute async sort keys before sorting or use a synchronous comparator that returns a number"
  sample_limit=3
  while IFS=$'\t' read -r sample_path sample_line sample_method sample_text; do
    [ -z "$sample_path" ] && continue
    print_code_sample "$sample_path" "$sample_line" "$sample_text"
    sample_limit=$((sample_limit - 1))
    [ "$sample_limit" -le 0 ] && break
  done <<<"$async_sort_comparator_samples"
fi

print_subheader "async callbacks passed to timers"
async_timer_report=$(python3 - "$PROJECT_DIR" <<'PY' 2>/dev/null
import os
import re
import sys
from pathlib import Path
...
async_timer_count=$(printf '%s\n' "$async_timer_report" | head -n1 | awk 'END{print $0+0}')
async_timer_samples=$(printf '%s\n' "$async_timer_report" | tail -n +2)
if [ "$async_timer_count" -gt 0 ]; then
  print_finding "warning" "$async_timer_count" "async timer callback is not awaited" "Wrap the async body in void run().catch(...) or handle rejections inside the timer callback"
  sample_limit=3
  while IFS=$'\t' read -r sample_path sample_line sample_method sample_text; do
    [ -z "$sample_path" ] && continue
    print_code_sample "$sample_path" "$sample_line" "$sample_text"
    sample_limit=$((sample_limit - 1))
    [ "$sample_limit" -le 0 ] && break
  done <<<"$async_timer_samples"
fi

print_subheader "async callbacks passed to event listeners"
async_listener_report=$(python3 - "$PROJECT_DIR" <<'PY' 2>/dev/null
import os
import re
import sys
from pathlib import Path
    ...
    for idx, line in enumerate(lines):
        ...
        for callback_idx in range(idx, min(len(lines), idx + 16)):
            ...
            if saw_listener and callback_idx > idx and paren_balance <= 0:
                break
        callback_text = ' '.join(callback_lines)
        if 'ubs:ignore' in callback_text or not async_listener_re.search(callback_text):
            continue
        try:
            rel = path.relative_to(sample_root)
        except ValueError:
...
async_listener_count=$(printf '%s\n' "$async_listener_report" | head -n1 | awk 'END{print $0+0}')
async_listener_samples=$(printf '%s\n' "$async_listener_report" | tail -n +2)
if [ "$async_listener_count" -gt 0 ]; then
  print_finding "warning" "$async_listener_count" "async event listener callback is not awaited" "Wrap the async body in void run().catch(...) or handle rejections inside the listener callback"
  sample_limit=3
  while IFS=$'\t' read -r sample_path sample_line sample_text; do
    [ -z "$sample_path" ] && continue
    print_code_sample "$sample_path" "$sample_line" "$sample_text"
    sample_limit=$((sample_limit - 1))
    [ "$sample_limit" -le 0 ] && break
  done <<<"$async_listener_samples"
fi

print_subheader "async callbacks passed to JSX event handlers"
async_jsx_handler_report=$(python3 - "$PROJECT_DIR" <<'PY' 2>/dev/null
import os
import re
import sys
from pathlib import Path
...
for path in candidates:
    ...
    for idx, line in enumerate(lines):
        ...
        brace_balance = 0
        saw_attribute = False
        for attr_idx in range(idx, min(len(lines), idx + 18)):
...
async_jsx_handler_count=$(printf '%s\n' "$async_jsx_handler_report" | head -n1 | awk 'END{print $0+0}')
async_jsx_handler_samples=$(printf '%s\n' "$async_jsx_handler_report" | tail -n +2)
if [ "$async_jsx_handler_count" -gt 0 ]; then
  print_finding "warning" "$async_jsx_handler_count" "async JSX event handler callback is not awaited" "Use a synchronous handler that starts async work with void run().catch(...) or handles errors internally"
  sample_limit=3
  while IFS=$'\t' read -r sample_path sample_line sample_text; do
    [ -z "$sample_path" ] && continue
    print_code_sample "$sample_path" "$sample_line" "$sample_text"
    sample_limit=$((sample_limit - 1))
    [ "$sample_limit" -le 0 ] && break
  done <<<"$async_jsx_handler_samples"
fi

print_subheader "async callbacks passed to EventEmitter listeners"
async_emitter_listener_report=$(python3 - "$PROJECT_DIR" <<'PY' 2>/dev/null
import os
import re
import sys
from pathlib import Path
...
async_emitter_listener_re = re.compile(
...
for path in candidates:
    ...
    for idx, line in enumerate(lines):
        ...
        if 'ubs:ignore' in callback_text:
            continue
        match = async_emitter_listener_re.search(callback_text)
        if not match:
            continue
        try:
            rel = path.relative_to(sample_root)
...
async_emitter_listener_count=$(printf '%s\n' "$async_emitter_listener_report" | head -n1 | awk 'END{print $0+0}')
async_emitter_listener_samples=$(printf '%s\n' "$async_emitter_listener_report" | tail -n +2)
if [ "$async_emitter_listener_count" -gt 0 ]; then
  print_finding "warning" "$async_emitter_listener_count" "async EventEmitter listener callback is not awaited" "Wrap the async body in void run().catch(...) or handle rejections inside the EventEmitter listener"
  sample_limit=3
  while IFS=$'\t' read -r sample_path sample_line sample_method sample_text; do
    [ -z "$sample_path" ] && continue
    print_code_sample "$sample_path" "$sample_line" "$sample_text"
    sample_limit=$((sample_limit - 1))
    [ "$sample_limit" -le 0 ] && break
  done <<<"$async_emitter_listener_samples"
fi

print_subheader "async Promise executors"
async_promise_executor_report=$(python3 - "$PROJECT_DIR" <<'PY' 2>/dev/null
import os
import re
import sys
from pathlib import Path
...
async_promise_executor_count=$(printf '%s\n' "$async_promise_executor_report" | head -n1 | awk 'END{print $0+0}')
async_promise_executor_samples=$(printf '%s\n' "$async_promise_executor_report" | tail -n +2)
if [ "$async_promise_executor_count" -gt 0 ]; then
  print_finding "warning" "$async_promise_executor_count" "async Promise executor drops thrown errors" "Do not pass an async function to new Promise; use a synchronous executor and handle async work with then/catch or an outer async function"
  sample_limit=3
  while IFS=$'\t' read -r sample_path sample_line sample_text; do
    [ -z "$sample_path" ] && continue
    print_code_sample "$sample_path" "$sample_line" "$sample_text"
    sample_limit=$((sample_limit - 1))
    [ "$sample_limit" -le 0 ] && break
  done <<<"$async_promise_executor_samples"
fi
...
print_subheader "dangerouslySetInnerHTML without sanitizer"
dangerously_set_html_report=$(python3 - "$PROJECT_DIR" <<'PY' 2>/dev/null
import os
import re
import sys
from pathlib import Path
...
dangerously_set_html_count=$(printf '%s\n' "$dangerously_set_html_report" | head -n1 | awk 'END{print $0+0}')
dangerously_set_html_samples=$(printf '%s\n' "$dangerously_set_html_report" | tail -n +2)
if [ "$dangerously_set_html_count" -gt 0 ]; then
  print_finding "warning" "$dangerously_set_html_count" "dangerouslySetInnerHTML without sanitizer" "Sanitize HTML with DOMPurify.sanitize() or sanitizeHtml() before rendering"
  sample_limit=3
  while IFS=$'\t' read -r sample_path sample_line sample_text; do
    [ -z "$sample_path" ] && continue
    print_code_sample "$sample_path" "$sample_line" "$sample_text"
    sample_limit=$((sample_limit - 1))
    [ "$sample_limit" -le 0 ] && break
  done <<<"$dangerously_set_html_samples"
fi
...
header_injection_count=$(printf '%s\n' "$header_injection_report" | head -n1 | awk 'END{print $0+0}')
header_injection_samples=$(printf '%s\n' "$header_injection_report" | tail -n +2)
if [ "$header_injection_count" -gt 0 ]; then
  print_finding "warning" "$header_injection_count" "Request-controlled value reaches HTTP response header" "Reject CR/LF and encode header values before setting response headers"
  sample_limit=3
  while IFS=$'\t' read -r sample_path sample_line sample_text; do
    [ -z "$sample_path" ] && continue
    print_code_sample "$sample_path" "$sample_line" "$sample_text"
    sample_limit=$((sample_limit - 1))
    [ "$sample_limit" -le 0 ] && break
  done <<<"$header_injection_samples"
fi

print_subheader "Request-derived filesystem paths"
path_traversal_report=$(python3 - "$PROJECT_DIR" <<'PY' 2>/dev/null
import os
import re
import sys
from pathlib import Path
...
path_traversal_count=$(printf '%s\n' "$path_traversal_report" | head -n1 | awk 'END{print $0+0}')
path_traversal_samples=$(printf '%s\n' "$path_traversal_report" | tail -n +2)
if [ "$path_traversal_count" -gt 0 ]; then
  print_finding "warning" "$path_traversal_count" "Request-derived path reaches file read/download/write sink" "Validate paths with path.resolve/path.relative containment checks or sanitize filenames before opening, sending, or saving files"
  sample_limit=3
  while IFS=$'\t' read -r sample_path sample_line sample_text; do
    [ -z "$sample_path" ] && continue
    print_code_sample "$sample_path" "$sample_line" "$sample_text"
...
print_subheader "JWT decode or verification bypass"
jwt_verification_report=$(python3 - "$PROJECT_DIR" <<'PY' 2>/dev/null
import os
import re
import sys
from pathlib import Path
...
jwt_verification_count=$(printf '%s\n' "$jwt_verification_report" | head -n1 | awk 'END{print $0+0}')
jwt_verification_samples=$(printf '%s\n' "$jwt_verification_report" | tail -n +2)
if [ "$jwt_verification_count" -gt 0 ]; then
  print_finding "warning" "$jwt_verification_count" "JWT decode/verify bypass risk" "Use jwt.verify()/jose.jwtVerify() with explicit algorithms, issuer, audience, and expiration checks before trusting claims"
  sample_limit=3
  while IFS=$'\t' read -r sample_path sample_line sample_text; do
    [ -z "$sample_path" ] && continue
    print_code_sample "$sample_path" "$sample_line" "$sample_text"
    sample_limit=$((sample_limit - 1))
    [ "$sample_limit" -le 0 ] && break
  done <<<"$jwt_verification_samples"
fi

print_subheader "CORS credentials with permissive origins"
cors_credentials_report=$(python3 - "$PROJECT_DIR" <<'PY' 2>/dev/null
import os
import re
import sys
from pathlib import Path
...
cors_credentials_count=$(printf '%s\n' "$cors_credentials_report" | head -n1 | awk 'END{print $0+0}')
cors_credentials_samples=$(printf '%s\n' "$cors_credentials_report" | tail -n +2)
if [ "$cors_credentials_count" -gt 0 ]; then
  print_finding "warning" "$cors_credentials_count" "Credentialed wildcard/reflected CORS" "Use an explicit trusted origin allowlist and emit Vary: Origin when Access-Control-Allow-Credentials is true"
  sample_limit=3
  while IFS=$'\t' read -r sample_path sample_line sample_text; do
    [ -z "$sample_path" ] && continue
    print_code_sample "$sample_path" "$sample_line" "$sample_text"
...
tls_verification_report=$(python3 - "$PROJECT_DIR" <<'PY' 2>/dev/null
import os
import re
import sys
from pathlib import Path
...
tls_verification_count=$(printf '%s\n' "$tls_verification_report" | head -n1 | awk 'END{print $0+0}')
tls_verification_samples=$(printf '%s\n' "$tls_verification_report" | tail -n +2)
if [ "$tls_verification_count" -gt 0 ]; then
  print_finding "warning" "$tls_verification_count" "TLS certificate verification disabled" "Keep certificate verification enabled; use explicit CA bundles for private trust roots"
  sample_limit=3
  while IFS=$'\t' read -r sample_path sample_line sample_text; do
    [ -z "$sample_path" ] && continue
    print_code_sample "$sample_path" "$sample_line" "$sample_text"
